import { TestBed } from '@angular/core/testing';
import { HttpClientTestingModule, HttpTestingController } from '@angular/common/http/testing';
import { ProductService } from './product.service';
import { Product } from '@interfaces/data.interface';
import { ApiSuccess } from '@interfaces/api-success.interface';
import { ApiError } from '@interfaces/api-error.interface';
import { environment } from 'src/environment/environment';

describe('ProductService', () => {
  let service: ProductService;
  let httpMock: HttpTestingController;
  const API_URL = environment.API_URL;

  // ---------------------- CONFIGURATION ----------------------
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      providers: [ProductService]
    });

    service = TestBed.inject(ProductService);
    httpMock = TestBed.inject(HttpTestingController);
  });

  afterEach(() => {
    httpMock.verify();
  });

  // ---------------------- INITIALIZATION ----------------------
  describe('Initialization', () => {
    it('should create the service', () => {
      expect(service).toBeTruthy();
    });
  });

  // ---------------------- GET PRODUCTS ----------------------
  describe('getProducts', () => {
    it('should return an array of products', () => {
      const mockProducts: Product[] = [
        { id: '1', name: 'Product 1', description: 'Desc', logo: '', date_release: new Date(), date_revision: new Date() }
      ];

      service.getProducts().subscribe(products => {
        expect(products.length).toBe(1);
        expect(products).toEqual(mockProducts);
      });

      const req = httpMock.expectOne(`${API_URL}/products`);
      expect(req.request.method).toBe('GET');
      req.flush({ data: mockProducts });
    });
  });

  // ---------------------- GET PRODUCT BY ID ----------------------
  describe('getProductById', () => {
    it('should return a product by ID', () => {
      const mockProduct: Product = { id: '1', name: 'Product 1', description: 'Desc', logo: '', date_release: new Date(), date_revision: new Date() };

      service.getProductById('1').subscribe(product => {
        expect(product).toEqual(mockProduct);
      });

      const req = httpMock.expectOne(`${API_URL}/products/1`);
      expect(req.request.method).toBe('GET');
      req.flush(mockProduct);
    });

    it('should return null if there is an error', () => {
      service.getProductById('999').subscribe(product => {
        expect(product).toBeNull();
      });

      const req = httpMock.expectOne(`${API_URL}/products/999`);
      req.error(new ErrorEvent('Not Found'), { status: 404 });
    });
  });

  // ---------------------- POST PRODUCT ----------------------
  describe('postProduct', () => {
    it('should create a product and return ApiSuccess', () => {
      const newProduct: Product = { id: '2', name: 'Product 2', description: 'Desc', logo: '', date_release: new Date(), date_revision: new Date() };
      const response: ApiSuccess<Product> = { message: 'Created', data: newProduct };

      service.postProduct(newProduct).subscribe(res => {
        expect(res).toEqual(response);
      });

      const req = httpMock.expectOne(`${API_URL}/products`);
      expect(req.request.method).toBe('POST');
      req.flush(response);
    });

    it('should handle 400 error in postProduct', () => {
      const newProduct: Product = { id: '2', name: 'Product 2', description: 'Desc', logo: '', date_release: new Date(), date_revision: new Date() };
      const apiError: ApiError = { message: 'Error', name: 'BadRequest', errors: [] };

      service.postProduct(newProduct).subscribe({
        next: () => fail('It should fail with error'),
        error: (err) => expect(err.message).toBe('Error')
      });

      const req = httpMock.expectOne(`${API_URL}/products`);
      req.flush(apiError, { status: 400, statusText: 'Bad Request' });
    });
  });

  // ---------------------- PUT PRODUCT ----------------------
  describe('putProduct', () => {
    it('should update a product', () => {
      const product: Product = { id: '1', name: 'Product 1', description: 'Desc', logo: '', date_release: new Date(), date_revision: new Date() };
      const response: ApiSuccess<Product> = { message: 'Updated', data: product };

      service.putProduct(product).subscribe(res => {
        expect(res).toEqual(response);
      });

      const req = httpMock.expectOne(`${API_URL}/products/1`);
      expect(req.request.method).toBe('PUT');
      req.flush(response);
    });

    it('should handle 400 error in putProduct', () => {
      const product: Product = { id: '1', name: 'Product 1', description: 'Desc', logo: '', date_release: new Date(), date_revision: new Date() };
      const apiError: ApiError = { message: 'Error', name: 'BadRequest', errors: [] };

      service.putProduct(product).subscribe({
        next: () => fail('It should fail with error'),
        error: (err) => expect(err.message).toBe('Error')
      });

      const req = httpMock.expectOne(`${API_URL}/products/1`);
      req.flush(apiError, { status: 400, statusText: 'Bad Request' });
    });
  });

  // ---------------------- DELETE PRODUCT ----------------------
  describe('deleteProduct', () => {
    it('should delete a product', () => {
      const response = { message: 'Deleted' };

      service.deleteProduct('1').subscribe(res => {
        expect(res).toEqual(response);
      });

      const req = httpMock.expectOne(`${API_URL}/products/1`);
      expect(req.request.method).toBe('DELETE');
      req.flush(response);
    });

    it('should handle 400 error in deleteProduct', () => {
      const apiError: ApiError = { message: 'Error', name: 'BadRequest', errors: [] };

      service.deleteProduct('1').subscribe({
        next: () => fail('It should fail with error'),
        error: (err) => expect(err.message).toBe('Error')
      });

      const req = httpMock.expectOne(`${API_URL}/products/1`);
      req.flush(apiError, { status: 400, statusText: 'Bad Request' });
    });
  });

  // ---------------------- VERIFY PRODUCT NAME ----------------------
  describe('verifyProductName', () => {
    it('should check if the name exists', () => {
      service.verifyProductName('1').subscribe(exists => {
        expect(exists).toBeTrue();
      });

      const req = httpMock.expectOne(`${API_URL}/products/verification/1`);
      expect(req.request.method).toBe('GET');
      req.flush(true);
    });

    it('should return false if it does not exist', () => {
      service.verifyProductName('999').subscribe(exists => {
        expect(exists).toBeFalse();
      });

      const req = httpMock.expectOne(`${API_URL}/products/verification/999`);
      expect(req.request.method).toBe('GET');
      req.flush(false);
    });
  });
});
